$(document).ready(function() {
	...
	$("#choiceCompleteBtn").on("click", function() {
		...
		$('<input>').attr({
			type: 'hidden',
			class: 'seatSelection1',
			value: seatNames1.join(',')
		}).appendTo('form');

		if (scheduleCount == 2) {
			$('<input>').attr({
				type: 'hidden',
				class: 'seatSelection2',
				value: seatNames2.join(',')
			}).appendTo('form');
		}
		...
		// 배열을 문자열로 변환하여 전달
		$(`input[name="seatNames"]`).val(seatNames1.join(','));
		...
		if (scheduleCount == 2) {
			...
			$(`input[name="seatNames2"]`).val(seatNames2.join(','));
...
function selectSeat(sch, type, seatName) {
	...
	if (type == 'e') {
		...
		if ($(`.seat--${sch}--${seatName}`).attr("src") == "/images/ticket/economy_not.png") {
			...
			if (sch == 1) {
				...
				seatCount2 --;
				// 취소된 좌석 정보를 hidden input에 즉시 업데이트
				$("input[name='seatNames2']").val(seatNames2.join(','));
			}
	...
	console.log("좌석 선택 상태 - 스케줄1:", seatNames1);
	console.log("좌석 선택 상태 - 스케줄2:", seatNames2);
	console.log("Hidden Input 값1:", $("input[name='seatNames']").val());
	console.log("Hidden Input 값2:", $("input[name='seatNames2']").val());
}
