...
import edu.du.airline_project.dto.nation.NationDto;
import edu.du.airline_project.dto.request.*;
...
public class UserController {
	...
	public String mainPage(Model model) {
		...
		if (allFaqList.isEmpty()) {
			// Handle the case where there are no FAQs (e.g., set indexArr to empty or some default values)
			model.addAttribute("indexArr", new int[0]); // or handle accordingly
		} else {
			for (int i = 0; i < indexArr.length; i++) {
				...
				for (int j = 0; j < i; j++) {
					if (indexArr[i] == indexArr[j]) {
						...
						break; // break here to ensure that we don't continue checking after a decrement
					}
				}
			}
