	...
	<div class="d-flex align-items-center flex-column" style="width: 100%;">
			...
			<div class="d-flex flex-column align-items-center" style="width: 430px">
				<ul class="info--ul">
					...
					<c:choose>
						...
						<c:when test="${ticketList.size() == 2}">
							<li>${ticketList.get(0).id} ㅣ ${ticketList.get(1).id}</li>
						</c:when>
						<%-- 편도인 경우 --%>
						<c:otherwise>
							<li>${ticketList.get(0).id}</li>
						</c:otherwise>
					</c:choose>
				</ul>
				...
				<ul class="info--ul">
					...
					<c:choose>
						<c:when test="${not empty ticketList}">
							<li>${ticketList.get(0).formatReservedDate()}</li>
						</c:when>
						<c:otherwise>
							<li>정보 없음</li>
						</c:otherwise>
				...
				<ul class="info--ul">
					...
					<c:choose>
						<%-- 왕복인 경우 --%>
						<c:when test="${ticketList.size() == 2}">
							<li>${ticketList.get(0).formatDepartureDate()} ~ ${ticketList.get(1).formatDepartureDate()}</li>
						</c:when>
						<%-- 편도인 경우 --%>
						<c:otherwise>
							<li>${ticketList.get(0).formatDepartureDate()}</li>
						</c:otherwise>
					</c:choose>
				</ul>
				...
				<ul class="info--ul">
					...
					<c:choose>
						<c:when test="${ticketList.size() == 2}">
							<li><fmt:formatNumber value="${ticketList.get(0).amount + ticketList.get(1).amount}" pattern="#,###"/>원</li>
						</c:when>
						<c:otherwise>
							<li><fmt:formatNumber value="${ticketList.get(0).amount}" pattern="#,###"/>원</li>
						</c:otherwise>
					</c:choose>
				</ul>
		...
		<button class="list--btn" onclick="location.href='/ticket/list/1';">
			<ul class="d-flex justify-content-center" style="margin: 0;">
				<li><span class="material-symbols-outlined" style="font-size: 23px; margin-right: 4px; margin-top: 2px;">inventory</span></li>
				<li style="font-size: 16px;">예약 내역 조회</li>
			</ul>
		</button>
