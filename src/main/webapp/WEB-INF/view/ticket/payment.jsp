					...
					<div style="width: 900px;">
						...
						<div>
							<div class="ticket--div" id="ticketDiv1">
								...
								<p class="flight--date">
									${sch1Info.strDepartureDate}&nbsp;~&nbsp;${sch1Info.strArrivalDate}&nbsp;(${sch1Info.flightTime})
								</p>
								<p class="seat--list">
							...
							<c:if test="${ticket.scheduleId2 != null}">
								...
								<div class="ticket--div" id="ticketDiv1">
									...
									<p class="flight--date">
										${sch2Info.strDepartureDate}&nbsp;~&nbsp;${sch2Info.strArrivalDate}&nbsp;(${sch1Info.flightTime})
									</p>
									<p class="seat--list">
						...
						<label id="checkCurrentUserLabel"><input type="checkbox" id="checkCurrentUser">&nbsp;&nbsp;예약자
							본인을 탑승객에 포함</label>
							...
							<table border="1">
								...
								<c:forEach var="i" begin="0" end="${ticket.adultCount - 1}">
									<tr class="passenger--tr" id="passenger${i + 1}">
										...
										<td class="gender--td">
											<input type="radio" value="M" name="gender${i + 1}" id="genderM${i + 1}"
												checked>
								...
								<table border="1">
									...
									<c:forEach var="i" begin="0" end="${ticket.childCount - 1}">
										<tr class="passenger--tr" id="passenger${ticket.adultCount + i + 1}">
											...
											<td class="gender--td">
												<input type="radio" value="M" name="gender${ticket.adultCount + i + 1}"
													id="genderM${ticket.adultCount + i + 1}" checked>
												<label for="genderM${ticket.adultCount + i + 1}">남성</label>
												&nbsp;&nbsp;
												<input type="radio" value="F" name="gender${ticket.adultCount + i + 1}"
													id="genderF${ticket.adultCount + i + 1}">
											...
											<td class="birth--td">
												<input type="text" name="birth${ticket.adultCount + i + 1}"
													placeholder="yyyy-mm-dd" value="2020-01-05">
											</td>
										</tr>
									</c:forEach>
								</table>
								...
								<table border="1">
									...
									<c:forEach var="i" begin="0" end="${ticket.infantCount - 1}">
										<tr class="passenger--tr"
											id="passenger${ticket.adultCount + ticket.childCount + i + 1}">
											<td class="age--td">
												유아 ${i + 1}
											</td>
											<td class="gender--td">
												<input type="radio" value="M"
													name="gender${ticket.adultCount + ticket.childCount + i + 1}"
													id="genderM${ticket.adultCount + ticket.childCount + i + 1}"
													checked>
												<label
													for="genderM${ticket.adultCount + ticket.childCount + i + 1}">남성</label>
												&nbsp;&nbsp;
												<input type="radio" value="F"
													name="gender${ticket.adultCount + ticket.childCount + i + 1}"
													id="genderF${ticket.adultCount + ticket.childCount + i + 1}">
												<label
													for="genderF${ticket.adultCount + ticket.childCount + i + 1}">여성</label>
											</td>
											<td class="name--td">
												<input type="text"
													name="name${ticket.adultCount + ticket.childCount + i + 1}"
													value="d">
											</td>
											<td class="birth--td">
												<input type="text"
													name="birth${ticket.adultCount + ticket.childCount + i + 1}"
						...
						<div class="d-flex justify-content-center" style="width: 100%; margin: 30px 0 40px;">
							<button class="blue--btn--middle" id="passengerInfoBtn">
								<ul class="d-flex justify-content-center">
									...
									<li><span class="material-symbols-outlined material-symbols-outlined-white"
											style="font-size: 25px; margin-top: 3px;">done</span>
								</ul>
							</button>
						</div>
						<!-- 여기서부터 결제 정보 -->
							...
							<div id="kakaoPayDiv">
								<table border="1" class="payment--table">
									...
									<tr class="payment--tb--depth--3">
										...
										<!-- 콤마 찍어서 나타내기 -->
										<td>
											<fmt:formatNumber value="${sch1AdultPrice}" pattern="#,###" />&nbsp;원
										</td>
									</tr>
									<c:if test="${ticket.childCount != 0}">
										<tr class="payment--tb--depth--3">
											...
											<td>
												<fmt:formatNumber value="${sch1ChildPrice}" pattern="#,###" />&nbsp;원
											</td>
										</tr>
									</c:if>
									<c:if test="${ticket.infantCount != 0}">
										<tr class="payment--tb--depth--3">
											...
											<td>
												<fmt:formatNumber value="${sch1InfantPrice}" pattern="#,###" />&nbsp;원
											</td>
										</tr>
									</c:if>
									...
									<c:if test="${ticket.scheduleId2 != null}">
										...
										<tr class="payment--tb--depth--3">
											...
											<!-- 콤마 찍어서 나타내기 -->
											<td>
												<fmt:formatNumber value="${sch2AdultPrice}" pattern="#,###" />&nbsp;원
											</td>
										</tr>
										<c:if test="${ticket.childCount != 0}">
											<tr class="payment--tb--depth--3">
												<td>소아 ${ticket.childCount}인</td>
												<td>
													<fmt:formatNumber value="${sch2ChildPrice}" pattern="#,###" />
													&nbsp;원
												</td>
											</tr>
										</c:if>
										<c:if test="${ticket.infantCount != 0}">
											<tr class="payment--tb--depth--3">
												<td>유아 ${ticket.infantCount}인</td>
												<td>
													<fmt:formatNumber value="${sch2InfantPrice}" pattern="#,###" />
													&nbsp;원
												</td>
											</tr>
										</c:if>
									...
									<tr>
										...
										<th>
											<c:choose>
												<c:when test="${ticket.scheduleId2 != null}">
													<fmt:formatNumber value="${ticket.price + ticket.price2}"
														pattern="#,###" />&nbsp;원
												</c:when>
												<c:otherwise>
								...
								<br>
								<button id="reserveButton" type="button">예약하기</button>
							...
							<div id="milesPayDiv">
								<table border="1" class="payment--table">
									...
									<tr class="payment--tb--depth--3">
										...
										<!-- 콤마 찍어서 나타내기 -->
										<td>
											<fmt:formatNumber value="${ticket.milesPrice(sch1AdultPrice)}"
												pattern="#,###" />&nbsp;마일
										</td>
									</tr>
									<c:if test="${ticket.childCount != 0}">
										<tr class="payment--tb--depth--3">
											<td>소아 ${ticket.childCount}인</td>
											<td>
												<fmt:formatNumber value="${ticket.milesPrice(sch1ChildPrice)}"
													pattern="#,###" />&nbsp;마일
											</td>
										</tr>
									</c:if>
									<c:if test="${ticket.infantCount != 0}">
										<tr class="payment--tb--depth--3">
											<td>유아 ${ticket.infantCount}인</td>
											<td>
												<fmt:formatNumber value="${ticket.milesPrice(sch1InfantPrice)}"
													pattern="#,###" />&nbsp;마일
											</td>
										</tr>
									</c:if>
									...
									<c:if test="${ticket.scheduleId2 != null}">
										...
										<tr class="payment--tb--depth--3">
											...
											<!-- 콤마 찍어서 나타내기 -->
											<td>
												<fmt:formatNumber value="${ticket.milesPrice(sch2AdultPrice)}"
													pattern="#,###" />&nbsp;마일
											</td>
										</tr>
										<c:if test="${ticket.childCount != 0}">
											<tr class="payment--tb--depth--3">
												<td>소아 ${ticket.childCount}인</td>
												<td>
													<fmt:formatNumber value="${ticket.milesPrice(sch2ChildPrice)}"
														pattern="#,###" />&nbsp;마일
												</td>
											</tr>
										</c:if>
										<c:if test="${ticket.infantCount != 0}">
											<tr class="payment--tb--depth--3">
												<td>유아 ${ticket.infantCount}인</td>
												<td>
													<fmt:formatNumber value="${ticket.milesPrice(sch2InfantPrice)}"
														pattern="#,###" />&nbsp;마일
												</td>
											</tr>
										</c:if>
									...
									<tr>
										...
										<th>
											<c:choose>
												<c:when test="${ticket.scheduleId2 != null}">
													<fmt:formatNumber
														value="${ticket.milesPrice(ticket.price) + ticket.milesPrice(ticket.price2)}"
														pattern="#,###" />&nbsp;마일
												</c:when>
												<c:otherwise>
													<fmt:formatNumber value="${ticket.milesPrice(ticket.price)}"
														pattern="#,###" />&nbsp;마일
												</c:otherwise>
											</c:choose>
										</th>
								...
								<br>
								<div id="milesBtnDiv">
									<button type="submit" id="milesPayBtn" class="blue--btn--small"
										onclick="return confirm('마일리지 결제를 진행하시겠습니까?');">
										<ul class="d-flex justify-content-center" style="margin: 0;">
											<li><span class="material-symbols-outlined material-symbols-outlined-white"
													style="font-size: 22px; margin-top: 3px;">flight</span>
