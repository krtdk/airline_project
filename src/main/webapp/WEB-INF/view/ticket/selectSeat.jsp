	...
	<div class="d-flex" style="width: 100%;">
		...
		<div style="width: 100%; margin-left: 20px" class="d-flex flex-column">
			...
			<div class="btn--div">
				...
				<form action="/ticket/payment" method="get" id="seatForm">
					<input type="hidden" name="adultCount" value="">
					<input type="hidden" name="childCount" value="">
					<input type="hidden" name="infantCount" value="">
					<input type="hidden" name="scheduleId" value="">
					<input type="hidden" name="seatNames" value="">
					<input type="hidden" name="seatGrade" value="">
					<!-- 왕복일 때에만 -->
					<c:if test="${ticketList.size() == 2}">
						<input type="hidden" name="scheduleId2" value="">
						<input type="hidden" name="seatNames2" value="">
						<input type="hidden" name="seatGrade2" value="">
					</c:if>
...
let airplaneId2;
let scheduleId2;
let seatNames2 = new Array();  // 수정된 부분
let seatGrade2;
let seatCount2 = 0;

// 폼 제출 처리
$("#choiceCompleteBtn").click(function(e) {
e.preventDefault();

// 좌석 선택 수 확인
if (scheduleCount === 1 && seatCount1 !== totalSeatCount) {
alert("좌석을 모두 선택해주세요.");
return;
}
if (scheduleCount === 2 && (seatCount1 !== totalSeatCount || seatCount2 !== totalSeatCount)) {
alert("좌석을 모두 선택해주세요.");
return;
}
...
// 기본 정보 설정
form.find('input[name="adultCount"]').val(adultCount);
form.find('input[name="childCount"]').val(childCount);
form.find('input[name="infantCount"]').val(infantCount);
form.find('input[name="scheduleId"]').val(scheduleId1);
form.find('input[name="seatGrade"]').val(seatGrade1);
form.find('input[name="seatNames"]').val(seatNames1.join(','));

// 왕복인 경우 추가 정보 설정
if (scheduleCount === 2) {
form.find('input[name="scheduleId2"]').val(scheduleId2);
form.find('input[name="seatGrade2"]').val(seatGrade2);
form.find('input[name="seatNames2"]').val(seatNames2.join(','));
}
